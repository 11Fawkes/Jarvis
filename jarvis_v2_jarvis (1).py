...
.sidebar-title { font-family: 'Fraunces', serif; font-size: 1.5rem; font-weight: 300; color: #f0ece4 !important; letter-spacing: -0.5px; line-height: 1.1; margin-bottom: 4px; }
.sidebar-sub { font-size: 0.55rem; letter-spacing: 2.5px; color: #2e2e2e !important; margin-bottom: 20px; }

[data-testid="stSidebar"] .stRadio label { font-size: 0.65rem !important; letter-spacing: 1.5px !important; color: #3a3a3a !important; padding: 9px 0 !important; cursor: pointer !important; text-transform: uppercase !important; border-bottom: 1px solid #161616 !important; display: block !important; }
...
[data-testid="stSidebar"] label[data-baseweb="radio"]:has(input:checked) { color: #E63946 !important; }
...
.obt-card::after { content:''; position:absolute; right:-40px; top:-40px; width:160px; height:160px; border-radius:50%; background:rgba(255,255,255,0.06); }
.obt-label { font-size:0.58rem; letter-spacing:2.5px; color:rgba(255,255,255,0.6); margin-bottom:10px; }
.obt-text { font-family:'Fraunces',serif; font-size:1.8rem; font-weight:300; color:#fff; line-height:1.2; }
.obt-reminder { font-size:0.62rem; color:rgba(255,255,255,0.5); margin-top:10px; letter-spacing:1px; }

.sec { font-size:0.58rem; letter-spacing:2.5px; color:#323232; text-transform:uppercase; margin:20px 0 10px; }
.sec-red { color:#E63946 !important; }
.sec-blue { color:#1D7FE8 !important; }
.sec-green { color:#22c55e !important; }
.sec-amber { color:#f59e0b !important; }

.task-row { display:flex; align-items:center; gap:12px; padding:10px 0; border-bottom:1px solid #202020; font-size:0.82rem; color:#aaa; }
...
.alive-label { font-size:0.85rem; color:#ddd9d2; font-weight:500; }
.alive-sub { font-size:0.62rem; color:#383838; margin-top:3px; letter-spacing:0.5px; }
.s-pill { font-size:0.55rem; letter-spacing:1.2px; padding:3px 9px; border-radius:4px; font-weight:500; text-transform:uppercase; flex-shrink:0; }
...
.stat-lbl { font-size:0.58rem; letter-spacing:2px; color:#323232; margin-bottom:6px; }
...
.win-item { display:flex; gap:10px; align-items:flex-start; padding:9px 0; font-size:0.8rem; color:#888; border-bottom:1px solid #202020; }
...
.habit-name { font-size:0.82rem; color:#aaa; }
...
.goal-track-lbl { font-size:0.82rem; color:#c8c3bc; margin-bottom:8px; }
.goal-track-sub { font-size:0.62rem; color:#323232; margin-top:6px; letter-spacing:1px; }

.log-detail { font-size:0.75rem; color:#484848; line-height:1.9; }
.log-detail span { color:#888; }
.blk-lbl { font-size:0.58rem; color:#1D7FE8; letter-spacing:1.5px; margin:10px 0 6px; }

.stTextArea textarea { background:#1c1c1c !important; border:1px solid #242424 !important; border-radius:10px !important; color:#c8c3bc !important; font-family:'DM Mono',monospace !important; font-size:0.78rem !important; }
.stTextArea textarea:focus { border-color:#E63946 !important; box-shadow:none !important; }
.stButton button { background:#E63946 !important; color:white !important; border:none !important; border-radius:8px !important; font-family:'DM Mono',monospace !important; font-size:0.65rem !important; letter-spacing:1.5px !important; width:100% !important; padding:12px !important; }

.streamlit-expanderHeader { background:#1c1c1c !important; border:1px solid #242424 !important; border-radius:10px !important; color:#c8c3bc !important; font-size:0.78rem !important; font-family:'DM Mono',monospace !important; }
...
def load_data():
    ...
    return {
        ...
        "weekly_wins":["First application sent","Consistent morning routine","Planned doctor visit"],
        "weekly_slipping":["Gym — skipped 3×","German study — 2 day gap"],
        "habits":[
...
def parse_log_block(text):
    ...
    for line in text.strip().split('\n'):
        ...
        for key,prefix in [('date','Date:'),('obt','One Big Thing:'),('must','Must:'),('should','Should:'),('can','Can:'),('carried','Carried forward:'),('week_goal','Week goal:'),('parked','Parked:'),('one_line','One line:')]:
...
def render_tasks(items_str,color):
    if not items_str or items_str.upper()=='NONE':
        return '<div style="font-size:0.75rem;color:#2a2a2a;padding:8px 0">Nothing here</div>'
    html=''
    for item in [i.strip() for i in items_str.split('·')]:
        done='✓' in item
        text=item.replace('✓','').replace('✗','').strip()
        chk=f'<span class="chk-done">✓</span>' if done else f'<span class="chk-open" style="border-color:{color}35"></span>'
        html+=f'<div class="task-row">{chk}<span class="{"task-done" if done else ""}">{text}</span></div>'
    return html

SC={'active':('#1D7FE8','#1D7FE810','#1D7FE825'),'pending':('#f59e0b','#f59e0b10','#f59e0b25'),'flagged':('#E63946','#E6394610','#E6394625'),'done':('#22c55e','#22c55e10','#22c55e25')}
...
with st.sidebar:
    ...
    <div style="margin:16px 0;padding:14px 16px;background:#1a1a1a;border-radius:10px;border:1px solid #1e1e1e">
        <div style="font-size:0.55rem;letter-spacing:2px;color:#2a2a2a;margin-bottom:4px">8-WEEK CHALLENGE</div>
        <div style="font-family:Fraunces,serif;font-size:2rem;font-weight:300;color:#E63946;line-height:1">Day {challenge_day}</div>
        <div style="font-size:0.55rem;color:#2a2a2a;letter-spacing:1px;margin-top:4px">{days_left} DAYS REMAINING</div>
        <div style="background:#1e1e1e;border-radius:99px;height:3px;margin-top:10px">
            <div style="background:#E63946;border-radius:99px;height:3px;width:{int(challenge_day/CHALLENGE_DAYS*100)}%"></div>
        </div>
    </div>
    ''',unsafe_allow_html=True)
    ...
    <div style="margin-top:24px;padding:12px 14px;background:#1a1a1a;border:1px solid #1e1e1e;border-radius:10px;display:flex;justify-content:space-between;align-items:center">
        <div style="font-size:0.55rem;color:#2a2a2a;letter-spacing:1.5px">CHAT</div>
...
if '⌂' in page:
    ...
    with c1:
        st.markdown(f'<div style="font-size:0.58rem;letter-spacing:2.5px;color:#2a2a2a;margin-bottom:8px">{date.today().strftime("%A · %B %d · %Y").upper()}</div>',unsafe_allow_html=True)
        ...
        if latest and latest.get('carried','').upper() not in ['','NONE']:
            ...
            st.markdown(f'<div class="j-card j-card-amber"><div style="font-size:0.82rem;color:#aaa">{latest.get("carried")}</div></div>',unsafe_allow_html=True)
    ...
    with c2:
        ...
        with b:
            st.markdown(f'<div class="stat-card"><div class="stat-lbl">WEEK GOAL</div><div class="stat-num" style="color:#1D7FE8">{wd}<span style="font-size:1rem;color:#2a2a2a">/{wt}</span></div><div style="font-size:0.6rem;color:#2a2a2a;margin-top:4px;letter-spacing:1px">APPLICATIONS</div></div>',unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════
    ...
    st.markdown(f'<div style="font-size:0.58rem;letter-spacing:2px;color:#2a2a2a;margin-bottom:20px">{date.today().strftime("%A · %B %d").upper()}</div>',unsafe_allow_html=True)
    ...
    if not latest:
        ...
        for f in ['must','should','can']:
            ...
            if v and v.upper()!='NONE':
                ...
                done_c+=sum(1 for i in items if '✓' in i)
        pct=int(done_c/total_c*100) if total_c>0 else 0
        st.markdown(f'<div style="font-size:0.65rem;color:#2a2a2a;letter-spacing:1px;margin-bottom:6px">{done_c} OF {total_c} DONE · {pct}%</div>',unsafe_allow_html=True)
        ...
        with c2:
            ...
            if latest.get('parked','').upper() not in ['','NONE']:
                ...
                st.markdown(f'<div class="j-card"><div style="font-size:0.82rem;color:#aaa">{latest.get("parked")}</div></div>',unsafe_allow_html=True)

        st.markdown('<hr>',unsafe_allow_html=True)
        st.markdown('<div class="j-card j-card-amber"><div style="font-size:0.58rem;color:#f59e0b;letter-spacing:2px;margin-bottom:8px">🌙 EVENING CHECK-IN</div><div style="font-size:0.8rem;color:#555;line-height:1.7">Finish your Claude chat. Type FINISH. Paste the block in the Update section. No guilt — just data and forward.</div></div>',unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════
    ...
    st.markdown('<div style="font-size:0.58rem;letter-spacing:2px;color:#2a2a2a;margin-bottom:20px">EVERYTHING OPEN IN YOUR LIFE RIGHT NOW</div>',unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════
    ...
    st.markdown(f'<div style="font-size:0.58rem;letter-spacing:2px;color:#2a2a2a;margin-bottom:20px">{wr.upper()} · 2026</div>',unsafe_allow_html=True)

    c1,c2=st.columns([1,2])
    with c1:
        theme=data.get('week_theme','Applications')
        st.markdown(f'<div class="j-card" style="text-align:center;padding:24px;border-color:#E6394620"><div style="font-size:0.58rem;letter-spacing:2.5px;color:#E63946;margin-bottom:10px">FOCUS THEME</div><div style="font-family:Fraunces,serif;font-size:3rem;font-weight:300;color:#f0ece4;line-height:1">{theme}</div></div>',unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="j-card j-card-green">{wins}</div>',unsafe_allow_html=True)
        st.markdown('<div class="sec sec-red">↩ SLIPPING</div>',unsafe_allow_html=True)
        slip=''.join([f'<div class="win-item"><span style="color:#E63946">↩</span>{w}</div>' for w in data.get('weekly_slipping',[])])
        st.markdown(f'<div class="j-card j-card-red">{slip}<div style="font-size:0.58rem;color:#2a2a2a;letter-spacing:1px;margin-top:10px">PATTERN: LOW ENERGY DAYS</div></div>',unsafe_allow_html=True)
    ...
    with c2:
        ...
        dots=''.join([f'<div style="text-align:center"><div style="font-size:0.55rem;color:#2a2a2a;margin-bottom:6px">{["M","T","W","T","F","S","S"][i]}</div><div style="width:12px;height:12px;border-radius:50%;background:{get_day_color(day_map.get(week_start+timedelta(days=i)))};margin:0 auto;{"box-shadow:0 0 8px #1D7FE8" if (week_start+timedelta(days=i))==date.today() else ""}"></div></div>' for i in range(7)])
...
# ═══════════════════════════════════════════════
    ...
    st.markdown('<div style="font-size:0.58rem;letter-spacing:2px;color:#2a2a2a;margin-bottom:20px">MAY 4 — JUN 28 · 2026 · 56 DAYS</div>',unsafe_allow_html=True)

    a,b,c,d=st.columns(4)
    for col,lbl,val,color in [(a,'CURRENT DAY',challenge_day,'#E63946'),(b,'DAYS LEFT',days_left,'#1D7FE8'),(c,'SESSIONS',len(chats),'#22c55e'),(d,'COMPLETE',f'{int(challenge_day/CHALLENGE_DAYS*100)}%','#f59e0b')]:
        with col:
    ...
    for chat in chats:
        try:
            d2=datetime.strptime(chat.get('date',''),'%A, %B %d %Y').date()
            day_map[d2]=chat
        except: pass

    grid='<div style="display:grid;grid-template-columns:repeat(8,1fr);gap:6px;margin-bottom:8px">'
    for w in range(1,9):
        grid+=f'<div style="font-size:0.52rem;color:#252525;letter-spacing:1px;text-align:center;padding-bottom:6px">WK{w}</div>'
    ...
    grid+='</div>'
    legend=''.join([f'<div style="display:flex;align-items:center;gap:5px"><div style="width:8px;height:8px;background:{c};border-radius:2px"></div><span style="font-size:0.55rem;color:#2a2a2a;letter-spacing:1px">{l}</span></div>' for c,l in [('#22c55e','STRONG'),('#f59e0b','PARTIAL'),('#E63946','MISSED'),('#1D7FE8','TODAY'),('#161616','AHEAD')]])
...
# ═══════════════════════════════════════════════
    ...
    st.markdown(f'<div style="font-size:0.58rem;letter-spacing:2px;color:#2a2a2a;margin-bottom:20px">{len(chats)} SESSIONS · YOUR COMPLETE RECORD</div>',unsafe_allow_html=True)

    if not chats:
        st.markdown('<div class="j-card" style="text-align:center;padding:60px"><div style="font-family:Fraunces,serif;font-size:1.5rem;color:#1e1e1e;font-style:italic">Your story starts tomorrow.</div><div style="font-size:0.6rem;color:#1a1a1a;margin-top:10px;letter-spacing:1.5px">CHAT 001 · DAY 1 · MAY 4 2026</div></div>',unsafe_allow_html=True)
    else:
        for chat in reversed(chats):
            cn=str(chat.get('chat_num','?')).zfill(3)
            cd=chat.get('date','Unknown date')
            cs=chat.get('one_line','')
            with st.expander(f"Chat {cn}  ·  {cd}",expanded=False):
                st.markdown(f'<div style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:0.95rem;color:#444;margin-bottom:12px">"{cs}"</div>',unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════
    ...
    st.markdown('<div style="font-size:0.58rem;letter-spacing:2px;color:#2a2a2a;margin-bottom:20px">PASTE YOUR LOG BLOCK FROM CLAUDE · ONE PASTE UPDATES EVERYTHING</div>',unsafe_allow_html=True)

    c1,c2=st.columns([2,1])
    with c1:
        log_input=st.text_area('',height=300,
        ...
        if st.button('UPDATE DASHBOARD →'):
            if log_input.strip():
                ...
                if parsed and 'obt' in parsed:
                    ...
                    cn2=parsed.get('chat_num',len(chats)+1)
                    idx=next((i for i,c in enumerate(chats) if c.get('chat_num')==cn2),None)
                    if idx is not None:
                    ...
                    data['chats']=chats
                    save_data(data)
                    st.success('✓ All sections updated')
                    st.rerun()
                else:
    ...
    with c2:
        st.markdown('<div class="j-card" style="border-color:#1D7FE820"><div style="font-size:0.58rem;color:#1D7FE8;letter-spacing:2px;margin-bottom:12px">HOW IT WORKS</div><div style="font-size:0.75rem;color:#444;line-height:1.9">1. End your Claude chat<br>2. Type <span style="color:#E63946">FINISH</span><br>3. Copy the block Claude generates<br>4. Paste it here<br>5. Hit update<br><br><span style="color:#282828">Tasks · Alive · Weekly · Challenge tracker · Log — all update from one paste.</span></div></div>',unsafe_allow_html=True)
        st.markdown('<div class="j-card j-card-amber" style="margin-top:10px"><div style="font-size:0.58rem;color:#f59e0b;letter-spacing:2px;margin-bottom:10px">LOG BLOCK FORMAT</div><div style="font-size:0.68rem;color:#3a3a3a;line-height:1.9;font-family:monospace">=== CHAT [N] · BLOCK [X.Y] ===<br>Date: Weekday, Mon DD YYYY<br>One Big Thing: ...<br>Must: item ✓ · item ✗<br>Should: item ✓ · item ✗<br>Can: item ✗<br>Carried forward: ...<br>Week goal: goal → X/Y<br>Parked: ...<br>One line: ...<br>=== END BLOCK ===</div></div>',unsafe_allow_html=True)
